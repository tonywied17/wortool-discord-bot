...
// Discord bot token and client ID from environment variables
const token = process.env.TOKEN;
const clientId = process.env.CLIENT_ID;
const bearerToken = process.env.AUTH_SECRET; // Your API authorization token for the backend
    ...
    try {
        ...
        for (const guildId of guildIds) {
            try {
                ...
                // Prepare roles data for the backend API
                const rolesData = roles.map(role => ({
                    id: role.id,
                    name: role.name,
                    // Add additional role properties here if needed
                }));
                ...
                // Replace this URL with your actual endpoint for syncing roles
